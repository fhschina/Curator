...
from eval.dedup.analysis import context_experiment as subject
...
from eval.dedup.judging.context_runtime import VERSION, encode_main_review
...
from tests.eval.dedup.test_composite_experiment import fixture as old_fixture
...
def fixture(stage):
    packets, values, mains = old_fixture(stage)
    for v in [*values.values(), *mains.values()]:
        v["contract_version"] = CONTRACT
    for packet in packets:
        if "proposed_main_text" in packet:
            packet["proposed_main_text"] = encode_main_review(mains[packet["canonical_pair_id"]])
    for reverse in (False, True):
        pid = "context_ba" if reverse else "context_ab"
        value, payload = context_certificate(reverse, prefix=pid.capitalize() + " probe. ")
        values[pid] = value
        packet = {
        ...
        if stage == "critic":
            main = deepcopy(value)
            main["hard_conflict"].update(kind="NONE", a_span_id="", b_span_id="")
            for field in ("a_meaning_in_b", "b_meaning_in_a"):
                main[field] = {
                    "status": "COVERED",
                    "reviewed_unique_ids": value[field]["reviewed_unique_ids"],
                    "harmless_unique_ids": "",
                    "opposite_support_ids": "S001",
                    "coverage_explanation": "Fallible main claim of equivalent membership.",
                }
            mains[pid] = main
            packet["proposed_main_text"] = encode_main_review(main)
...
def test_fresh_main_critic_attribution_never_reuses_old_predictions(tmp_path):
    ...
    rows, _ = subject.ContextArm("main").replay_cell(root, "main", "coverage", packets, {})
    labels = [
        {
...
def test_frozen_manifest_and_budget_guard(tmp_path):
    ...
    artifact.write_text("{}")
    with pytest.raises(DedupEvaluationError, match="CONTEXT_FREEZE"):
    ...
    with pytest.raises(DedupEvaluationError, match="COMPOSITE_TOKEN_BUDGET"):
        subject.ContextArm("main", tokenizer=Tokenizer()).layout(fixture("main")[0], {}, "coverage")
    with pytest.raises(DedupEvaluationError, match="COMPOSITE_MAIN_REUSE"):
        subject.ContextArm("main").layout(fixture("main")[0], {"old": {}}, "coverage")
...
def test_native_ray_http_writer_binder_first_and_retry(stage, retry, httpserver, tmp_path):
    ...
    arm = subject.ContextArm(stage)
    packets, values, mains = fixture(stage)
    called, owned = arm.layout(packets, mains, "coverage")
    for packet in called:
        packet["repair_feedback"] = encode_repair_feedback(
            {"code": "CONTEXT_SHARED_WITNESS", "message": "Retry", "details": {}} if retry else None
        )
    ...
    settings = _settings() | {"logical_model": "context-boundary", "ray_temp_dir": "/raid/hfang/ihb/r31diag"}
