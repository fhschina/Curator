...
"""Frozen Exp6 composition plus narrowly scoped coverage-format recovery."""
...
from eval.dedup.analysis import exp1_conflict_evidence_fix as previous
from eval.dedup.judging import coverage_format_recovery
from eval.dedup.judging import critic_anchor_ids_v2 as critic_anchor_ids
from eval.dedup.judging import main_repetition_recovery_v2 as repetition
from eval.dedup.validation import DedupEvaluationError, require, sha256_file, sha256_json, write_json_atomic

VERSION = "v0.6.2.33-exp6+coverage-format-fix1"
old = previous.old
...
def execute_case(root: Path, row: dict, endpoint: str, frozen: dict, render_coverage: Callable) -> dict:
    ...
    def review(stage: str, messages: list[dict], schema: dict) -> dict:
        request = old.body(messages, schema)
    ...
    try:
        ...
        for outer in range(1, 4):
            ...
            try:
                ...
                raw = old.main_toolchain()[1].parse(choice["message"]["content"]).model_dump()
                public = old.common.critic.main_decision(raw, payload)
        ...
        if old.common.critic.route(public, payload) == "REVIEW_POSITIVE_DIRECTIONS_ONLY":
            ...
            normalized, corrections = critic_anchor_ids.normalize_shared_anchor_ids(payload, raw)
            if corrections:
                result["coverage_anchor_id_repair"] = {
                    "contract_version": critic_anchor_ids.CONTRACT
                    if any(c["original"].isdigit() for c in corrections)
                    else critic_anchor_ids.previous.CONTRACT,
            ...
            try:
                ...
                repaired = review(
                    ...
                    previous.repair_messages(payload, raw, render_coverage),
                    old.coverage.response_schema(),
                )
                result["coverage_evidence_repair"]["repaired_review"] = deepcopy(repaired)
                require(
                    set(repaired) == set(raw)
                    and all(repaired[k] == v for k, v in raw.items() if k not in previous.EDITABLE_FIELDS),
