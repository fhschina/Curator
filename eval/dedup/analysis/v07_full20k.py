...
"""Fresh internal v0.7 collection with a smoke gate and nohup continuation."""
...
CHECKPOINT_VERSION = "v0.6.2.33-exp6+main-repetition-fix2"
...
def select_smoke(rows: list[dict], controls: list[str], history: dict[str, dict]) -> list[dict]:
    ...
    for key in controls:
        ...
        if key not in seen:
            ...
            seen.add(key)
    require(len(selected) == len(seen) == SMOKE_SIZE, "V07_SMOKE_PANEL", "unique frozen smoke population")
    return selected
...
def verify(root: Path) -> dict:
    ...
    require(
        ...
        "V07_RUNTIME_BINDING",
        "the frozen v0.7 runtime and fresh answers are required",
    )
    return manifest
...
def prepare(root: Path) -> dict:
    ...
    recovery.intact(checkpoint["runtime_version"] == CHECKPOINT_VERSION)
    ...
    extras = [
        HERE,
        HERE.with_name("v07.py"),
        PROTOCOL,
        Path(runtime.candidate.__file__),
        Path(runtime.critic_anchor_ids.__file__),
        Path(runtime.coverage_format_recovery.__file__),
        *[
    ...
    manifest = {
        ...
        "mode": "INTERNAL_V07_FRESH_FULL20K_WITH_SMOKE_GATE",
...
def replay_results(root: Path, rows: list[dict]) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="v07-replay-") as directory:
        ...
        def replay(path: Path, key: str, body: dict, endpoint: str) -> dict:
            recovery.intact(path == target and key not in seen and endpoint == "offline://v07")
    ...
    with recovery.exclusive(root):
        ...
        try:
            ...
            with limits.RateLimitRelay(
                ...
                relay.set_context(
                    recovery.original.RelayContext("v07-" + session.name, 0, root / "transport_events.jsonl")
                )
...
def launch(root: Path, env_file: Path, *, smoke_only: bool = False) -> dict:
    with recovery.exclusive(root):
        ...
        argv = [
            ...
            "-u",
            "-m",
            "eval.dedup.analysis.v07_full20k",
            "run",
            "--root",
            str(root),
            "--env-file",
