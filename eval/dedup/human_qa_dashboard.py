...
<header class="topbar"><div><h1>Human QA Review</h1><div class="subtitle">Run __RUN_ID__ · reviewer-blind packet · __VERSION__</div></div><div class="actions"><label class="packet-select">Review set<select id="packet-selector" aria-label="Review set"></select></label><button class="button" id="import-button" type="button">Import CSV</button><input class="hidden" id="import-file" type="file" accept=".csv,text/csv"><button class="button" id="export-backup" type="button">Export backup JSON</button><button class="button primary" id="export-csv" type="button">Export labels CSV</button></div></header>
...
const FIELDS=["same_duplicate_group","a_can_replace_b","b_can_replace_a","relation_type","material_difference","fuzzy_scope"],REQUIRED_FIELDS=FIELDS.slice(0,3),CSV_FIELDS=["qa_pair_id",...FIELDS,"reason_codes","reviewer_status","notes"];
...
function csvEscape(value){const text=String(value??"");return /[",\n\r]/.test(text)?`"${text.replaceAll('"','""')}"`:text}function exportRows(){return PAIRS.map(pair=>{const rv=review(pair.qa_pair_id),state=stateFor(pair.qa_pair_id),row={qa_pair_id:pair.qa_pair_id};for(const field of FIELDS)row[field]=state==="PENDING"||state==="DRAFT"?rv[field]||"":rv[field];row.reason_codes=rv.reason_codes.length?JSON.stringify(rv.reason_codes):"";row.reviewer_status=state==="DRAFT"?"PENDING":state;row.notes=rv.notes||"";return row})}function download(name,type,text){const url=URL.createObjectURL(new Blob([text],{type})),link=create("a");link.href=url;link.download=name;document.body.appendChild(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(url),0)}function exportCsv(){const lines=[CSV_FIELDS.join(","),...exportRows().map(row=>CSV_FIELDS.map(field=>csvEscape(row[field])).join(","))];download(`${packet.label}_labels.csv`,"text/csv;charset=utf-8",lines.join("\r\n")+"\r\n")}function exportBackup(){download(`${packet.label}_review_backup.json`,"application/json",JSON.stringify({dashboard_version:VERSION,run_id:RUN_ID,packet_digest:packet.digest,reviews},null,2))}
