...
PAIR_EXPLORER_VERSION = "dedup-pair-explorer-v2"
...
<script>
const PAIRS=__PAIR_DATA__,GROUPS=__GROUP_CONTEXT_DATA__,RUN_ID=__RUN_ID_JSON__,VERSION=__EXPLORER_VERSION_JSON__,PAGE_SIZE=100,STORE=`dedup-pair-reviews:${RUN_ID}:${VERSION}`,PAIR_IDS=new Set(PAIRS.map(x=>x.pair_id)),VALID_STATUS=new Set(["AGREE_WITH_JUDGE","DISAGREE_WITH_JUDGE","UNSURE"]),VALID_VERDICT=new Set(["","DUPLICATE","NOT_DUPLICATE","UNRESOLVED"]);
const ids=["search","track","outcome","relation","material","scope","group","retriever","evidence","review","hostname"],controls=Object.fromEntries(ids.map(id=>[id,document.getElementById(id)])),tbody=document.getElementById("pair-rows"),detail=document.getElementById("detail"),state={page:0,filtered:[],selected:null};let reviews=loadReviews();
...
function fill(id,field){for(const v of[...new Set(PAIRS.map(r=>r[field]).filter(Boolean))].sort()){const o=create("option","",v);o.value=v;controls[id].appendChild(o)}}for(const v of[...new Set(PAIRS.flatMap(r=>r.outcomes))].sort()){const o=create("option","",v);o.value=v;controls.outcome.appendChild(o)}fill("relation","relation_type");fill("material","material_difference");fill("scope","fuzzy_scope");fill("group","group_size_bucket");fill("retriever","retriever_category");
function loadReviews(){try{return JSON.parse(localStorage.getItem(STORE)||"{}")}catch(_){return{}}}function persist(){try{localStorage.setItem(STORE,JSON.stringify(reviews));document.getElementById("storage-status").textContent="Reviews saved in this browser."}catch(_){document.getElementById("storage-status").textContent="Storage unavailable; export before closing."}}function review(id){return reviews[id]||{review_status:"",human_verdict:"",notes:"",updated_at:""}}function reviewed(id){return VALID_STATUS.has(review(id).review_status)}
function matches(r){const q=controls.search.value.trim().toLowerCase(),rv=review(r.pair_id),search=[r.pair_id,r.left.doc_id,r.right.doc_id,r.left.predicted_cluster_key,r.right.predicted_cluster_key,r.left.hostname,r.right.hostname,...r.reason_codes,...r.error_types].join(" ").toLowerCase(),rm=!controls.review.value||(controls.review.value==="UNREVIEWED"?!reviewed(r.pair_id):controls.review.value==="REVIEWED"?reviewed(r.pair_id):rv.review_status===controls.review.value);return(!q||search.includes(q))&&(!controls.track.value||(controls.track.value==="5a"?r.has_5a:r.has_5b))&&(!controls.outcome.value||r.outcomes.includes(controls.outcome.value))&&(!controls.relation.value||r.relation_type===controls.relation.value)&&(!controls.material.value||r.material_difference===controls.material.value)&&(!controls.scope.value||r.fuzzy_scope===controls.scope.value)&&(!controls.group.value||r.group_size_bucket===controls.group.value)&&(!controls.retriever.value||r.retriever_category===controls.retriever.value)&&(!controls.evidence.value||r.judge_evidence_status.coverage===controls.evidence.value)&&rm&&(!controls.hostname.value||(controls.hostname.value==="same")===r.same_hostname)}
...
for(const c of Object.values(controls))c.addEventListener(c.id==="search"?"input":"change",applyFilters);document.getElementById("reset").onclick=()=>{for(const c of Object.values(controls))c.value="";applyFilters()};document.getElementById("previous").onclick=()=>{if(state.page){state.page--;renderRows()}};document.getElementById("next").onclick=()=>{state.page++;renderRows()};document.getElementById("copy-id").onclick=()=>navigator.clipboard.writeText(state.selected);document.getElementById("export-csv").onclick=exportCsv;document.getElementById("export-json").onclick=exportJson;document.getElementById("import-reviews").onclick=()=>document.getElementById("review-file").click();document.getElementById("review-file").onchange=e=>{if(e.target.files[0])importFile(e.target.files[0])};window.onhashchange=()=>{const m=location.hash.match(/^#pair=(.+)$/);if(m)selectPair(decodeURIComponent(m[1]),false)};const track=new URLSearchParams(location.search).get("track");if(track)controls.track.value=track;applyFilters();const initial=location.hash.match(/^#pair=(.+)$/);if(initial)selectPair(decodeURIComponent(initial[1]),false);
</script></body></html>"""
    return (
