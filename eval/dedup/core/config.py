...
from eval.dedup.core.validation import DedupEvaluationError, assert_exact_keys, require, sha256_json
...
def _load_judge(value: dict[str, Any], base: Path) -> AnyJudgeConfig:
    if value.get("backend") == "local_ndd":
        ...
        require(
            config.schema_version == "dedup-judge-output-v3"
            and config.visible_payload_version == "judge-visible-payload-v3",
            "INVALID_JUDGE_CONFIG",
            "the stable local judge uses output and visible-payload schema v3",
        )
        require(config.num_gpus > 0, "INVALID_JUDGE_CONFIG", "local_ndd requires at least one GPU")
    else:
        fields = set(JudgeConfig.__dataclass_fields__)
        assert_exact_keys(value, fields, context="judge")
        config = JudgeConfig(**value)
    require(
        bool(config.prompt_version)
        and config.schema_version
        in {"dedup-judge-output-v0", "dedup-judge-output-v1", "dedup-judge-output-v2", "dedup-judge-output-v3"},
        "INVALID_JUDGE_CONTRACT",
        "judge prompt identity and a supported output schema are required",
...
def load_config(path: str | Path, *, path_base: Path | None = None) -> EvaluationConfig:
    ...
    if config.retrieval.backend == "fixture_cpu":
        ...
        require(
            ...
            "INVALID_RETRIEVAL_BACKEND",
            "retrieval backend must be fixture_cpu or gpu_cudf",
        )
    require(
        all(value >= 0 for value in config.seeds.values()),
        "INVALID_SEED",
        "evaluation seeds must be non-negative integers",
    )
    require(
        config.output_root != config.source_path.parent and config.cache_root != config.source_path.parent,
        "SOURCE_TREE_OUTPUT",
        "run output and cache roots must not be the configuration source directory",
    )
    return config
