...
from eval.dedup.judging.output_schema import flatten_reason_codes

PAIR_EXPLORER_VERSION = "dedup-pair-explorer-v4"
...
def build_pair_explorer_records(run_root: Path, rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Join Judge, SUT endpoint, sampling, and visible-text context for the explorer."""
    ...
    for row in sorted(rows, key=lambda item: str(item["canonical_pair_id"])):
        ...
        record = {
            ...
            "primary_material_difference": result.get("primary_material_difference")
            or row.get("primary_material_difference"),
            "expected_minhash_action": result.get("expected_minhash_action") or row.get("expected_minhash_action"),
            "surface_evidence_sufficiency": result.get("surface_evidence_sufficiency")
            or row.get("surface_evidence_sufficiency"),
            "expected_minhash_outcome": result.get("expected_minhash_outcome") or row.get("expected_minhash_outcome"),
            "dominant_overlap_source": result.get("dominant_overlap_source") or row.get("dominant_overlap_source"),
            "primary_risk_factor": result.get("primary_risk_factor") or row.get("primary_risk_factor"),
            "evidence_quality": result.get("evidence_quality") or row.get("evidence_quality"),
            "confidence_tier": result.get("confidence_tier") or row.get("confidence_tier"),
...
def pair_explorer_review_queue(
    ...
    """Keep actionable pairs and report controls in the static explorer."""
                ...
                "value": (
                    ...
                    and (
                        ...
                        or any(
                            ...
                            in {
                                "BOILERPLATE",
                                "PRIMARY_RISK:BOILERPLATE_DOMINATED_SIMILARITY",
...
function summary(r){const c=create("section","decision-flow");card(c,"SUT decision",r.sut_grouping_result,r.has_5a?`Removed ${r.right.doc_id}; kept ${r.left.doc_id}`:"No removal between this pair",r.sut_grouping_result==="DIFFERENT_GROUPS"?"tone-warn":"");card(c,"Judge verdict",r.judge_group_verdict,r.has_5a?`Deletion safety: ${r.judge_removal_verdict}`:`Confidence: ${value(r.confidence_tier||r.confidence)}`);card(c,"Evaluation outcome",r.evaluation_outcome,raw(r),outcomeTone(r));detail.appendChild(c)}
...
function judge(r){const s=section("Judge verdict","Primary duplicate/replacement decisions are scored separately from optional MinHash diagnostics."),f=create("div","facts"),items=[["Judge group verdict",r.judge_group_verdict],["Same duplicate group",r.same_duplicate_group],[`${r.left_role} can replace ${r.right_role}`,r.left_can_replace_right],[`${r.right_role} can replace ${r.left_role}`,r.right_can_replace_left],["Relation",r.relation_type],["Material difference",r.material_difference],["Primary material difference",r.primary_material_difference],["Legacy fuzzy scope",r.fuzzy_scope],["Expected MinHash action",r.expected_minhash_action],["Surface evidence sufficiency",r.surface_evidence_sufficiency],["Expected MinHash outcome",r.expected_minhash_outcome],["Dominant overlap source",r.dominant_overlap_source],["Primary risk factor",r.primary_risk_factor],["Evidence quality",r.evidence_quality],["Confidence tier",r.confidence_tier],["Legacy numeric confidence",r.confidence],["Reason codes",r.reason_codes.join(", ")||"None"],["Judge errors",r.error_types.join(", ")||"None"]];if(r.has_5a)items.splice(2,0,["Deletion safety",r.judge_removal_verdict]);for(const x of items)fact(f,...x);s.appendChild(f)}
