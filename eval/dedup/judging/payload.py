...
from eval.dedup.core.config import AnyJudgeConfig
from eval.dedup.core.contracts import canonical_json_bytes
...
def assert_blind_payload(payload: dict[str, Any]) -> None:
    ...
    if payload.get("payload_schema_version") == VISIBLE_PAYLOAD_V2:
        ...
        for kind, prefix in (("SHARED", "S"), ("A_ONLY", "A"), ("B_ONLY", "B")):
            expected_ids.extend(
                f"{prefix}{index:03d}" for index in range(1, min(int(counts[kind]), _MAX_DIFF_SPANS_PER_KIND) + 1)
            )
