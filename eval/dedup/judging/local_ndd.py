...
def _adapt_v0(value: dict[str, Any]) -> dict[str, Any]:
    parsed: dict[str, Any] = {}
    for field in _V0_FIELDS:
        score = _score(value, field)
    ...
    parsed["evidence"] = []
    return validate_judge_output(parsed, JUDGE_SCHEMA_V0)


def _adapt_v2(value: dict[str, Any], payload: dict[str, Any] | None) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    parsed: dict[str, Any] = {}
    ...
    for field in (
        ...
        require(isinstance(score, str), "LOCAL_NDD_OUTPUT_INVALID", "categorical score must be text", field=field)
        parsed[field] = score.upper()
    parsed["surface_evidence_sufficiency"], parsed["expected_minhash_outcome"] = derive_minhash_diagnostics(
        parsed["same_duplicate_group"], parsed["expected_minhash_action"]
    )
    confidence = _score(value, "confidence")
    confidence_key = str(confidence)
    require(
    ...
    parsed["reason_codes"].append(f"EVIDENCE_STATUS:{parsed['evidence_quality']}")
    if parsed["same_duplicate_group"] == "UNRESOLVED":
        parsed["evidence"] = []
        evidence_events = []
    else:
        parsed["evidence"], evidence_events = _extract_quote_evidence(value, payload)
    return validate_judge_output(parsed, JUDGE_SCHEMA_V2), [
        {
...
def _adapt_v3(
    ...
    return validate_judge_output(parsed, JUDGE_SCHEMA_V3), [
        {
            "action": "derive_same_duplicate_group",
            "source_fields": ["a_can_replace_b", "b_can_replace_a"],
        },
...
def _adapt_ndd_judge_output_with_events(
    ...
    *,
    ...
    if schema_version == JUDGE_SCHEMA_V2:
        return _adapt_v2(value, payload)
    require(
        schema_version == JUDGE_SCHEMA_V0,
        "LOCAL_NDD_OUTPUT_INVALID",
        "NDD adapter does not support the configured output schema",
        schema_version=schema_version,
    )
    return _adapt_v0(value), []
    ...
    *,
    ...
    return {
        ...
        "deterministic_repair_version": (
            ...
            else "v062-asymmetric-record-binding-arbitration-and-span-evidence-v7"
            if config.judge.prompt_version == HS_V0629_PROMPT_VERSION
            else "v062-calibrated-record-binding-critic-and-span-evidence-v6"
            if config.judge.prompt_version == HS_V0628_PROMPT_VERSION
            else "v062-record-binding-critic-and-span-evidence-v5"
            if config.judge.prompt_version == HS_V0627_PROMPT_VERSION
            else "v062-semantic-directional-derivation-evidence-align-and-exhaustion-v4"
            if config.judge.schema_version == JUDGE_SCHEMA_V3
            else (
                "v061-directional-derivation-and-unresolved-evidence-align-v3"
                if config.judge.schema_version == JUDGE_SCHEMA_V2
                else "visible-evidence-align-v1"
            )
        ),
        **parsed,
    }
...
def run_local_ndd_pending(
    ...
    *,
    ...
    last_raw_judge: dict[str, dict[str, Any]] = {}
        ...
        for attempt in range(1, config.judge.max_retries + 2):
            ...
            for pair_id in remaining:
                ...
                try:
                    ...
                    judge_column = (
                        SEMANTIC_JUDGE_COLUMN if config.judge.schema_version == JUDGE_SCHEMA_V3 else JUDGE_COLUMN
                    )
                    raw_judge = output_row.get(judge_column)
                    record_binding_critic = None
                    raw_provider_response = raw_judge
                    if config.judge.prompt_version in {
                        HS_V0627_PROMPT_VERSION,
                        HS_V0628_PROMPT_VERSION,
                        HS_V0629_PROMPT_VERSION,
                        HS_V06210_PROMPT_VERSION,
                        ...
                        require(
                            isinstance(record_binding_critic, dict),
                            "LOCAL_NDD_OUTPUT_INVALID",
                            "the selected prompt requires the independent record-binding critic",
                            field=RECORD_BINDING_CRITIC_COLUMN,
                        )
                        raw_provider_response = {
                            "semantic_judge": raw_judge,
                            "record_binding_critic": record_binding_critic,
                        }
                    if isinstance(raw_provider_response, dict):
                        last_raw_judge[pair_id] = raw_provider_response
                    parsed, repair_events = _adapt_ndd_judge_output_with_events(
                        raw_judge,
                        config.judge.schema_version,
                        payload=prepared[pair_id],
                        record_binding_critic=record_binding_critic,
                    ...
                    persist(
                        _result_record(
                            row=by_id[pair_id],
                            parsed=parsed,
                            raw_judge=raw_provider_response,
                            config=config,
    ...
    if config.judge.schema_version == JUDGE_SCHEMA_V3:
        exhausted_contract_conflicts = []
        for pair_id in remaining:
            pair_errors = errors[pair_id]
            last_issue = pair_errors[-1].get("validation_issue", {}) if pair_errors else {}
            raw_judge = last_raw_judge.get(pair_id)
            if last_issue.get("code") != "JUDGE_CONSISTENCY_INVALID" or raw_judge is None:
                continue
            persist(
                _result_record(
                    row=by_id[pair_id],
                    parsed=unresolved_judge_output_v3(),
                    raw_judge=raw_judge,
                    config=config,
                    contract_digest=contract_digest,
                    contract_version=contract_version,
                    attempt=config.judge.max_retries + 1,
                    repair_events=[
                        {
                            "action": "downgrade_exhausted_contract_conflict_to_unresolved",
                            "source_issue": last_issue.get("message"),
                            "failed_attempts": len(pair_errors),
                        }
                    ],
                )
            )
            exhausted_contract_conflicts.append(pair_id)
        exhausted = set(exhausted_contract_conflicts)
        remaining = [pair_id for pair_id in remaining if pair_id not in exhausted]
